...
## Setup serial comm to arduino 
try:
        ard = serial.Serial('/dev/ttyACM1', baudrate = 115200) # PORT MARKED "ARD"
except:
        ard = serial.Serial('/dev/ttyACM0', baudrate = 115200) # PORT MARKED "ARD"
...
try:
    while(1):        
        ...
        ## Get temp and humidity from arduino USB serial 

        try:
                ard_temp_humid = ard.readline().decode("utf-8")
                ard_temp_humid = ard_temp_humid.split(', ')
                ard_temp = float(ard_temp_humid[0].strip())
                ard_humid = float(ard_temp_humid[1].strip())
        ...
        print("Time:" + str(day) + "/" + str(month) + " - " + str(hour) + ":" + str(minute) + " | Room temp:" + str(room_temp) + " | Outside temp:" + str(outside_temp) + " | Set temp:" + str(set_temp) + " | Arduino Humidity:" + str(ard_humid) + "    | To exit press 'ctrl + c'")


        #log data
        with open('log_cyclic.csv', 'a') as log:
            writer = csv.writer(log)
            # record data to csv file
            #header = ['Date', 'Time', 'Room temp', 'Set temp', 'Outside temp', RH]
            data = [str(day)+"/"+str(month)+"/"+str(year), str(hour)+":"+str(minute), room_temp, set_temp, outside_temp, ard_humid]
            writer.writerow(data)
            # close the log file
            log.close()


        time.sleep(60) ### FIX THIS TO BE REGULAR INTERVALS 
